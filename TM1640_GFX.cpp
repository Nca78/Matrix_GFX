...
void TM1640_GFX::init()
{
  ...
  pinMode(_clk, OUTPUT);
  // enable (go out of shutdown)
  sendCommand(TM1640_CMD_DISPLAY_ON);
...
void TM1640_GFX::setBrightness(uint8_t level)
{
  ...
  sendCommand(TM1640_CMD_BRIGHTNESS + (level & 0x07));
}
...
/**
    ...
    @param  index
            ...
            Default = TM1640_ALLMATRIX, will apply to all matrix connected
*/
void TM1640_GFX::sendCommand(uint8_t command, uint16_t value)
{
  // From datasheet: Start transfer, set data and clk pin low
  digitalWrite(_data, LOW);
  customShiftOut(command);
  if (value != TM1640_NOVALUE)
  {
    customShiftOut(lowByte(value));
  }
...
void TM1640_GFX::display()
{
  ...
  // Set address at 0
  sendCommand(TM1640_CMD_ADDRESS_SET);
  // From datasheet: Start transfer, set data and clk pin low
  digitalWrite(_data, LOW);
